...
public class ReservationDAO extends BaseDAO {
	...
	// Calculate reservation price including amenities
	public String fetchCalculations(String bookingDates, int guests, int hotelRoomId, List<Integer> hotelAmts) {
		...
		total = total.add(roomHolidayAddition);
	...
	public String saveReservation(Map<String, String> data, int userId) {
		...
		try {
			...
			String sqlh = "INSERT INTO reservations (guests, check_in, check_out, nights, total, booked_date, user_id, hotel_room_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
			...
			ps.setDate(2, Date.valueOf(LocalDate.parse(bookingDates[0].trim(), DateFormatUtil.getFormatter())));
			ps.setDate(3, Date.valueOf(LocalDate.parse(bookingDates[1].trim(), DateFormatUtil.getFormatter())));
